...
| Driver | Block |
|--------|-------|
| `docker` | `[build.docker]` — `image`, `platforms` (single), `dockerfile`, `env_file`, `load=true`, `pull=true` (set `false` / `--pull=false` to skip base-image registry HEAD), `cache_bust`, `provenance` / `sbom` (optional bool; `false` → `--provenance=false` / `--sbom=false`, recommended for Aliyun ACR) |
