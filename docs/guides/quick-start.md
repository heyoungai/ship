...
`pin = "digest"` 时，compose 镜像行应使用 `@${APP_IMAGE_DIGEST}`；若仍按 `:tag` 拉取，请改用 `pin = "tag"`（v2.7.1 起也会自动降级并警告）。digest pin 复盘见 [hotfix-v2.7.1-digest-pin.md](../changes/completed/hotfix-v2.7.1-digest-pin.md)。
