...
- Status: **fixed**（hotfix 线：`v2.7.0` → `v2.7.1`）
- Date: 2026-07-24
- Branch: `hotfix/v2.7.1`
- Base tag: `v2.7.0`
- Fix commit: `ae0b394`（以最终 tag 上的 commit 为准）

> 本文供发布复盘与后续排查。合回主干时，可迁到 `docs/changes/completed/`（若该目录已存在）。
...
| 情况 | 做法 |
|------|------|
| master 仍缺同等修复 | `cherry-pick` 本 hotfix 提交（不要整支 merge） |
| master 已有等价修复 | 不必合回；保留本分支 / tag 作考古 |
| 文档结构已分层 | 将本文移到 `docs/changes/completed/hotfix-v2.7.1-digest-pin.md` 并更新索引 |
